...
public class ReadPixel extends Component {
	...
	public void generateChainCode(int row, int col, int w, int h) {
		...
		System.out.println("Starting at pixel " + row + " " + col);
		binaryArray[row][col] = 4;

		while(true) {
			System.out.println(Arrays.deepToString(binaryArray).replace("], ", "]\n"));

				if (binaryArray[startRow][startCol + 1] == 1) { // EAST
					System.out.println(0 + " East");
					binaryArray[startRow][startCol + 1] = 2;
					chaincode += 0;
					startCol += 1;

				} else if (binaryArray[startRow + 1][startCol + 1] == 1) { // SOUTH EAST
					System.out.println(1 + " South East");
					binaryArray[startRow + 1][startCol + 1] = 2;
					chaincode += 7;
					startRow += 1;
					startCol += 1;

				} else if (binaryArray[startRow + 1][startCol] == 1) { //  SOUTH
					System.out.println(3 + " South");
					binaryArray[startRow + 1][startCol] = 2;
					chaincode += 6;
					startRow += 1;

				} else if (binaryArray[startRow + 1][startCol - 1] == 1) { //  SOUTH WEST
					System.out.println(3 + " South West");
					binaryArray[startRow + 1][startCol - 1] = 2;
					chaincode += 5;
					startRow += 1;
					startCol -= 1;
				} else if (binaryArray[startRow][startCol - 1] == 1) { // WEST
					System.out.println(4 + "West");
					binaryArray[startRow][startCol - 1] = 2;
					chaincode += 4;
					startCol -= 1; 
				} else if (binaryArray[startRow - 1][startCol - 1] == 1) { // NORTH WEST
					System.out.println(5 + " North West");
					binaryArray[startRow - 1][startCol - 1] = 2;
					chaincode += 3;
					startRow -= 1;
					startCol -= 1; 
				} else if (binaryArray[startRow - 1][startCol] == 1) { //  NORTH
					System.out.println(6 + "North");
					binaryArray[startRow - 1][startCol] = 2; 
					chaincode += 2;
					startRow -= 1; 
				} else if (binaryArray[startRow - 1][startCol + 1] == 1) { // NORTH EAST
					System.out.println(7 + " North East");
					binaryArray[startRow + 1][startCol - 1] = 2;
					chaincode += 1;
					startRow -= 1;
					startCol += 1;
				...
				System.out.println("\n Start Row: " + startRow + " and startCol" + startCol);
